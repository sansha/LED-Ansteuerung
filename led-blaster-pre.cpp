...
#include </home/pi/LED-Ansteuerung/led-blaster-pre.hpp>
using namespace std;
...
//	int mode = 0;
	...
	while(true) {
			...
			if (confFile)
			{
				...
				//now change modes
				 if (strcmp(exit_cmd, fileInputString)==0) {
			  		printf("exit program. thank you. \n");
			  		fclose(confFile);
					printf("clear confFile\n");
					...
					if (confFile)
					{
			  		...
			  		gpioTerminate();
			  		return 0;
			  	} else if (strcmp(mode_cmd, fileInputString)==0) {
			  		sscanf (fileInputValue, " = %d", &value);
			  		cout << value << endl; 
			  		mode = value;
			  	} else if (strcmp(wait_cmd, fileInputString)==0) {
			  		sscanf (fileInputValue, " = %d", &value);
			  		cout << value << endl; 
			  		wait = value;
			  		waitCounter = value;
			  	} else if (strcmp(whtb_cmd, fileInputString)==0) {
			  		sscanf (fileInputValue, " = %d", &value);
			  		cout << value << endl; 
			  	...
			  	} else if (strcmp(redb_cmd, fileInputString)==0) {
			  		sscanf (fileInputValue, " = %d", &value);
			  		cout << value << endl; 
			  	...
			  	} else if (strcmp(grnb_cmd, fileInputString)==0) {
			  	...
			  	} else if (strcmp(blub_cmd, fileInputString)==0) {
			 		sscanf (fileInputValue, " = %d", &value);
			  		cout << value << endl; 
			...
			/*if (wait == 0 || waitCounter == 0) {  //if wait == 0 it should fade direct after a change, else if the waitCounter is 0 the wait is over -> fade 
				fadeSuccessively(fadeDelayUs, targetBrightness);
				wait = 0;
			}*/
	
			gpioDelay(10000); //so it won't be too fast ;)
	  	...
	  	if (mode==1) {
	  		...
	  		int a = pthread_create(&continiousFadeThread, NULL, modeContFadePointer , randomPointer);
	  		//modeContiniousFade();
